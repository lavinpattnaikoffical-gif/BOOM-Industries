...
async function check() {
  try {
    ...
    if (last.rows[0]) console.log('Last Standard:', last.rows[0].name, '-', last.rows[0].requirement, '-', last.rows[0].created_at);
    if (lastEvent.rows[0]) console.log('Last Event:', lastEvent.rows[0].name, '-', lastEvent.rows[0].event_type, '-', lastEvent.rows[0].created_at);
  } catch (err) {
